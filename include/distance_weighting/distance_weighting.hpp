...
public:
    using max_val_t = std::vector<float>;
    using max_point_t = std::vector<cv::Point>;

public:
    auto distanceWeighting(const cv::Mat &in){
        cv::Mat weighting(cv::Mat::zeros(in.size(), CV_32FC1));
        ...
        for(auto i(1); i < (num_rows - 1); i++){		
            ...
            float *weighting_ptr1 = weighting.ptr<float>(i-1);
            float *weighting_ptr2 = weighting.ptr<float>(i);
            float *weighting_ptr3 = weighting.ptr<float>(i+1);
            for(auto j(1); j < (num_cols - 1); j++){
                if(in_ptr2[j]){
                    if(in_ptr3[j-1] !=0 && in_ptr3[j] !=0 && in_ptr3[j+1] !=0 && 
                        ...
                        float min_dist = fminf(fminf(weighting_ptr2[j-1], weighting_ptr1[j-1]),
    ...
    auto search(const cv::Mat& in){
        ...
        for(std::size_t i(0); i < contours.size(); i++){
            ...
            if(r.area() < 400){ //-- different treatment
                ...
                max_val.push_back(static_cast<float>(maxv));                
            }else {
